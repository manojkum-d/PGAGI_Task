...
export default function CampaignTable() {
      ...
      case "actions":
        return (
          <div className="flex justify-end items-center gap-2">
            {" "}
            {/* Use justify-end to align to end */}
  ...
  const classNames = React.useMemo(
    () => ({
      wrapper: ["max-h-[382px]", "max-w-[400px]"], // Change the width here
    ...
    >
      <TableHeader columns={headerColumns}>
        {(column: any) => (
          <TableColumn
            key={column.uid}
            align={column.uid === "actions" ? "end" : "start"} // Align center for "actions"
            allowsSorting={column.sortable}
            style={{ width: column.uid === "actions" ? "100px" : "auto" }}
          >
            {column.name}
          </TableColumn>
        )}
